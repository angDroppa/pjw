...
import { BiciclettaCatalog, TipologiaBicicletta, Alimentazione, CreateSpecificheSchema, UpdateSpecificheSchema } from '@/lib/zodSchemas/bicicletta'
...
const ALIMENTAZIONI: Alimentazione[] = ['MUSCOLARE', 'ELETTRICA']
...
export default function BiciclettePage() {
  ...
  const [specAlim,   setSpecAlim]   = useState<Alimentazione>('MUSCOLARE')
  ...
  const createBici = async () => {
    ...
    setSavingMod(true)
    try {
      await backofficeApi.createBicicletta({ nome: newNome.trim(), tipologia: newTipol })
  ...
  const updateSpec = async () => {
    ...
    const payload = {
      ...
      ...(editSpec.alimentazione ? { alimentazione: editSpec.alimentazione }          : {}),
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Field label="Alimentazione">
              <div style={{ display: 'flex', gap: 16 }}>
                {ALIMENTAZIONI.map(a => (
                  <label key={a} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', color: 'var(--text-primary)', fontSize: 13 }}>
                    <input type="radio" name="alim" value={a} checked={specAlim === a}
                      onChange={() => setSpecAlim(a)}
                      style={{ accentColor: 'var(--accent)' }}
                    />
                    {a === 'MUSCOLARE' ? 'Muscolare' : 'Elettrica'}
                  </label>
                ))}
              </div>
            </Field>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Field label="Alimentazione">
              <div style={{ display: 'flex', gap: 16 }}>
                {ALIMENTAZIONI.map(a => (
                  <label key={a} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', color: 'var(--text-primary)', fontSize: 13 }}>
                    <input type="radio" name="edit-alim" value={a} checked={editSpec.alimentazione === a}
                      onChange={() => setEditSpec(e => e && ({ ...e, alimentazione: a }))}
                      style={{ accentColor: 'var(--accent)' }}
                    />
                    {a === 'MUSCOLARE' ? 'Muscolare' : 'Elettrica'}
                  </label>
...
const LABEL_ALIM: Record<string, string> = {
  MUSCOLARE: 'Muscolare', ELETTRICA: 'Elettrica',
}

function BiciCard({ bici, onAddSpec, onDelete, onDeleteSpec, onEditSpec }: {
  bici: BiciclettaCatalog
  onAddSpec: () => void
  onDelete: () => void
  onDeleteSpec: (id: number) => void
  onEditSpec: (s: BiciclettaCatalog['specifics'][number]) => void
}) {
  return (
    <Card>
      <div style={{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'var(--font-body)', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              {['Taglia', 'Alim.', '€/gg', '€/½gg', 'Altezza', ''].map(h => (
                <th key={h} style={{
            ...
            {bici.specifics.map(s => (
              <tr key={s.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '8px 10px', color: 'var(--text-primary)' }}>
                  {LABEL_ALIM[s.alimentazione] ?? s.alimentazione}
                </td>
                <td style={{ padding: '8px 10px', color: 'var(--text-primary)' }}>
